...
void Solve() {
    const long long MODC = 998244353LL;
    read(n); 
    readv(a, n); 
    readv(b, n); 
    ...
    for (int i = 1; i < n; i++) {
        ...
        for (int prev = 0; prev < 2; prev++) {
            long long prev_a = (prev ? b[i-1] : a[i-1]);
            long long prev_b = (prev ? a[i-1] : b[i-1]);
            for (int cur = 0; cur < 2; cur++) {
                long long cur_a = (cur ? b[i] : a[i]);
                long long cur_b = (cur ? a[i] : b[i]);
                if (prev_a <= cur_a && prev_b <= cur_b) {
                    dp[i][cur] += dp[i-1][prev];
                    if (dp[i][cur] >= MODC) dp[i][cur] -= MODC * ((dp[i][cur]) / MODC);
                }
            }
        }
    }

    long long ans = (dp[n-1][0] + dp[n-1][1]) % MODC;
    cout << ans << nn;
}
